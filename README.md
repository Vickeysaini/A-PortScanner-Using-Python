...
PortScanner is an open source penetration testing tool that 
...
You can download PortScanner by cloning the [Git](https://github.com/medbenali/PortScanner) repository:

    git clone https://github.com/medbenali/PortScanner.git
    cd PortScanner/
    python PortScanner.py -v

PortScanner works out of the box with [Python](http://www.python.org/download/) version **2.6.x** and **2.7.x**. 

# The PortScanner Module Usage


[PortScanner](https://github.com/medbenali/PortScanner)  is able to send and capture packets of several protocols, forging and decoding them to be used to most network tasks such as scanning, pinging, probing, and attacks.

Make sure you have PortScanner in your machine:

```sh
$ git clone https://github.com/medbenali/PortScanner.git
```

You can test the installation firing up PortScanner iteratively. These are some useful functions:
...
We can perform **ping** operations with several protocols using PortScanner The fastest way to discover hosts on a local Ethernet network is to use ARP:

### ARP Ping

```sh
$ PortScanner -s 192.168.1.0/24 -p arp
...
```sh
$ PortScanner -s 192.168.1.1-254 -p icmp
[*] Starting Ping ARP for 192.168.1.0/24
Begin emission:
Finished to send 256 packets.
...
```sh
$ PortScanner -s 192.168.1.1-254 -p tcp -d 80
```
...
```sh
$ PortScanner -s 192.168.*.1-10 -p udp
```
...
```sh
$ PortScanner -s 192.168.1.1 -p scan -d 1 -t 100
WARNING: No route found for IPv6 destination :: (no default route?)
[*] PortScanner Port Scanner
[*] Scanning 192.168.1.1 From Port 1 To 100: 
[*] Starting PortScanner 1.01 at 2017-07-14 14:00 CEST
[*] Scan In Progress ...
[*] Connecting To Port :  100 
[*] Scanning Completed at 2017-07-14 14:00 CEST
[*] PortScanner done: 1IP address (1host up) scanned in 0.32 seconds
[*] Open Ports: 
	23 TELNET: Open
	53 DNS: Open
	80 HTTP: Open
...
```sh
$ PortScanner -s 8.8.8.8 -p scan
WARNING: No route found for IPv6 destination :: (no default route?)
[*] PortScanner Port Scanner
[*] Scanning For Most Common Ports On 8.8.8.8
[*] Starting PortScanner 1.01 at 2017-07-14 14:03 CEST
[*] Scan In Progress ...
...
[*] PortScanner done: 1IP address (1host up) scanned in 13.11 seconds
[*] Open Ports: 
	53 DNS: Open
	443 HTTPS: Open
```
...
```sh
$ PortScanner -s 72.229.28.185 -p geoip
WARNING: No route found for IPv6 destination :: (no default route?)
...
PortScanner can analyse pcap files in order to extract and decode ethernet, ip, tcp, icmp, udp headers.

### Ethernet Headers

```sh
$ PortScanner -f test.pcap -p eth
WARNING: No route found for IPv6 destination :: (no default route?)
----------------------------------------
[*] Packet : 1
[+] ### [ Ethernet ] ###
...
```sh
$ PortScanner -f test.pcap -p icmp
WARNING: No route found for IPv6 destination :: (no default route?)
----------------------------------------
[*] Packet : 1
[+] ###[ ICMP ] ###
